...
public class FinanceWindows extends JPanel {
	...
	public void setComponent() {
		...
		UpRightArea.add(new JLabel("总收入：305,000"));
		UpRightArea.add(new JLabel("总支出：237,000"));
	...
	public JFreeChart createChart(CategoryDataset categoryDataset) {
		JFreeChart jfreechart = ChartFactory.createLineChart("收入统计图", // 标题
				...
				false, // tooltips
				false); // URLs
		jfreechart.getTitle().setFont(new Font("微软雅黑", Font.BOLD, 15));//设置标题 
		//设置图例类别字体           
		jfreechart.getLegend().setItemFont(new Font("微软雅黑", Font.BOLD, 15));  
		return jfreechart;
	}
	/*
	 * @初始化表格
